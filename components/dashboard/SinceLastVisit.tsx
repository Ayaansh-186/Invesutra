...
function Delta({ label, current, previous, suffix = "" }: {
  label: string;
  current: number;
  previous: number;
  suffix?: string;
  ...
  return (
    <div className="flex-1 min-w-[88px]">
      ...
      <div className={`mt-0.5 flex items-center gap-1 text-sm font-semibold ${tone}`}>
        <Icon className="h-3.5 w-3.5 shrink-0" />
        {flat ? "no change" : `${diff > 0 ? "+" : ""}${diff}${suffix}`}
      </div>
    </div>
  );
}
...
export default function SinceLastVisit({
  ...
  const valueDiff = currentValue - Number(previous.total_value);
  ...
  return (
    <div className="mb-6 rounded-2xl border border-[var(--shell-border)] bg-[var(--shell-surface)] p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        {streak >= 2 && (
          <span className="flex shrink-0 items-center gap-1 rounded-full bg-amber-400/15 px-2.5 py-1 text-[11px] font-semibold text-amber-500">
          ...
          >
            {Math.abs(valueDiff) < 1 ? (
              ...
              <>
                ...
                {valueDiff > 0 ? "+" : "−"}
                {formatCurrency(Math.abs(valueDiff), true)}
              </>
            )}
          </div>
        </div>

        <Delta label="Health" current={currentHealth} previous={Number(previous.health_score)} />
        <Delta label="Risk" current={currentRisk} previous={Number(previous.risk_score)} />
      </div>
    </div>
  );
}
